...
trx_fsn = trx.filter((col(conformed.Transactions.bu.name) == 'fsn_new')
    ...
    .select("idi_counterparty_gr", "dat_date_type_1", "cua_amount_type_1", "idi_turnover", "bu", "cod_sor_gr",
            "des_country_name", "des_maf_brand_name").cache()

# defining flowing dataframe
Flowing_df = trx_fsn.select(conformed.Transactions.idi_counterparty_gr.name).distinct().cache()
Flowing_df.show()
...
Flowing_df = add_active_inactive_flag(trx_fsn, "THAT", six_months_ago,
                                      "lifestyle_that_status_active_ksa", "lifestyle_that_status_inactive_ksa",
                                      Flowing_df)
Flowing_df = add_active_inactive_flag(trx_fsn, "LuluLemon (Store)", six_months_ago,
                                      "lifestyle_lll_status_active_ksa", "lifestyle_lll_status_inactive_ksa",
                                      Flowing_df)
Flowing_df = add_active_inactive_flag(trx_fsn, "All Saints (Store)", six_months_ago,
                                      "lifestyle_als_status_active_ksa", "lifestyle_als_status_inactive_ksa",
                                      Flowing_df)
...
Flowing_df = add_active_inactive_flag(trx_fsn, "Abercrombie and Fitch (Store)", six_months_ago,
                                      "lifestyle_anb_status_active_ksa", "lifestyle_anb_status_inactive_ksa",
                                      Flowing_df)
Flowing_df = add_active_inactive_flag(trx_fsn, "Crate & Barrel (Store)", six_months_ago,
                                      "lifestyle_cnb_status_active_ksa", "lifestyle_cnb_status_inactive_ksa",
                                      Flowing_df)
Flowing_df = add_active_inactive_flag(trx_fsn, "CB2 (Store)", six_months_ago,
                                      "lifestyle_cb2_status_active_ksa", "lifestyle_cb2_status_inactive_ksa",
                                      Flowing_df)
Flowing_df = add_active_inactive_flag(trx_fsn, "Fashion For Less (Store)", six_months_ago,
                                      "lifestyle_ffl_status_active_ksa", "lifestyle_ffl_status_inactive_ksa",
                                      Flowing_df)
...
expressions = [col(col_name) if col_name not in rounded_columns else round(when(col(col_name).isNotNull(),
                                                                                col(col_name)), 2).alias(col_name) for
               col_name in Flowing_df.columns]
...
count_exprs = [count(col(c)).alias(c) for c in Flowing_df.columns]
print("count_exprs---------------", count_exprs)
count_df = Flowing_df.select(count_exprs)

# Collect the results
count_values = count_df.collect()[0].asDict()

# Print column names and their non-null counts
for column, count_value in count_values.items():
    print(f"Column: {column}, Non-Null Count: {count_value}")
